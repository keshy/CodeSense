...
def main():
    ...
    with st.expander("📂 Directory Status"):
        ...
        # Cleanup option
    st.sidebar.markdown("---")
    if st.sidebar.button("🗑️ Clean Up Directories"):
        try:
            if os.path.exists("resources/code"):
                shutil.rmtree("resources/code")
            if os.path.exists("resources/cf"):
                shutil.rmtree("resources/cf")
            create_directories()
            st.sidebar.success("Directories cleaned up!")
            st.experimental_rerun()
        except Exception as e:
            st.sidebar.error(f"Error cleaning up: {e}")
