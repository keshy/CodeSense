...
def get_call_from_func_element(func):
    ...
    """
    try:
        assert type(func) in (ast.Attribute, ast.Name, ast.Subscript, ast.Call, ast.IfExp, ast.BinOp)
    except AssertionError as e:
        print('Caught assertion error!' + str(e))
        return None
...
class Python(BaseLanguage):
    ...
    @staticmethod
    def is_library_file(filename, _):
        """
        Get the file name and check if this is part of a library that's outside of the user code base.
        For now supporting pip and toml based detections where files are part of site-packages
    ...
    @staticmethod
    def make_nodes(tree, parent, filename):
        """
        ...
        func_source = None
        if raw_code:
            func_source = ast.get_source_segment(raw_code, tree)
        ...
        return [Node(token, calls, variables, parent, import_tokens=import_tokens, line_number=line_number,
                     is_constructor=is_constructor, source_code=func_source)]
    ...
    def make_root_node(lines, parent):
        ...
        return Node(token, calls, variables, parent=parent, line_number=line_number)
    ...
    def make_class_group(tree, parent):
        ...
        for node_tree in node_trees:
            class_group.add_node(Python.make_nodes(node_tree, parent=class_group, filename=None)[0])
