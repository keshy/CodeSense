...
class PHP(BaseLanguage):
    ...
    def make_nodes(tree, parent, filename):
        ...
        node = Node(token, calls, variables, parent, import_tokens=import_tokens, line_number=lineno(tree),
                    is_constructor=is_constructor)

        subnodes = flatten([PHP.make_nodes(t, parent, filename) for t in subnode_trees])
    ...
    def make_class_group(tree, parent):
        ...
        for node_tree in node_trees:
            for new_node in PHP.make_nodes(node_tree, parent=class_group, filename=None):
